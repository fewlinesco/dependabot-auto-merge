...
var NotDependabotPrError = class extends Error {
  constructor() {
    super();
    this.message = "\u{1F4A4} Skipping: This is not a Dependabot PR.";
  }
};
var NotValidSemverError = class extends Error {
};
...
var WrongInputError = class extends Error {
  constructor(input) {
    super();
...
async function approve({ repo, prNumber }) {
...
async function squashAndMerge({ repo, prNumber }) {
  const octokit = getClient();
...
async function askForReview({ repo, prNumber }, reviewers, message) {
  const octokit = getClient();
  const body = "\u{1F6A7} Manual check needed \u{1F6A7}" + (message ? "\n\n**" + message + "**" : "") + "\n\n" + reviewers.map((reviewer) => `@${reviewer}`).join(" ");
...
function diff(from, to) {
  let releaseType;
  try {
    releaseType = import_semver.default.diff(from, to);
  } catch (error) {
...
function get(text) {
  if (!import_semver.default.valid(text)) {
    throw new NotValidSemverError();
  } else if (import_semver.default.prerelease(text)) {
    throw new UnsupportedFeatureError();
  } else {
    return {
      full: text,
      major: import_semver.default.major(text),
      minor: import_semver.default.minor(text),
      patch: import_semver.default.patch(text)
    };
  }
}
